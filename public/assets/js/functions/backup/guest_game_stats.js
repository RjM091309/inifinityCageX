$(document).ready(function () {
	...
	// Get the agency and guest parameters from the URL
	const agency = getQueryParam('agency'); // This will fetch 'ERIC' from the URL if it's present
	const guest = getQueryParam('guest'); // This will fetch 'KIM,HYUNJONG' from the URL if it's present
...
if (backToGuestsLink) {
    if (agency && agency !== 'null') {
        backToGuestsLink.href = `/guest_statistic?agency=${encodeURIComponent(agency)}`;
        console.log(`Back link set to: /guest_statistic?agency=${agency}`);
    } else {
        // If no agency parameter found, set to the default guest statistic page
        backToGuestsLink.href = '/guest_statistic';
        console.log('No agency parameter found, redirecting to /guest_statistic');
    }
} else {
    console.error('Element with id "backToGuests" not found.');
}
	
	
	
	function reloadData() {
		const url = '/guest_game_statistics_data?agency=' + encodeURIComponent(agency) + '&guest=' + encodeURIComponent(guest);
		$.ajax({
			url: '/guest_game_statistics_data?agency=' + encodeURIComponent(agency) + '&guest=' + encodeURIComponent(guest), // Endpoint to fetch data
			method: 'GET',
					...
					$.ajax({
						...
						//	var net = (total_rolling_chips * (row.COMMISSION_PERCENTAGE / 100)).toLocaleString();
							...
							totalWinLoss += parseFloat(winloss.replace(/,/g, ''));
								...
								var houseShareValueLive = localStorage.getItem('houseShareLive_' + row.game_list_id) || '60';

							var houseShareLive = `<div class="btn-group">
								<input type="text" class="form-control text-center house-share-live-input" data-rowindex="${rowIndex}" value="${houseShareValueLive}" style="font-size: 12px !important;" readonly/>
							</div>`;

						
							// Ensure winloss and houseShare are numbers for calculation
		
							var houseSharePercentageLive = parseFloat(houseShareValueLive / 100); // Convert houseShare to number

							console.log('houseSharePercentageLive:', houseSharePercentageLive);

							// Calculate NGR: (winloss * (houseShare / 100)) - formattedNet
							var ngr = (WinLoss * houseSharePercentageLive) - net;
