$(document).ready(function () {
	...
	function reloadData() {
		$.ajax({
			url: '/guest_statistics_data?agency=' + encodeURIComponent(agency), // Endpoint to fetch data
			method: 'GET',
			success: function (data) {
				...
				data.forEach(function (row) {
					...
					let request = $.ajax({
						...
						var houseSharePercentageLive = parseFloat(houseShareValueLive) / 100;
				...
				$.when.apply($, requests).done(function () {
					...
					for (let agent in agentData) {
						...
						dataTable.row.add([
							`<a href="/guest_game_statistic?agency=${encodeURIComponent(aggregated.agency_name)}&guest=${encodeURIComponent(agent)}">${agent}</a>`, // Make agent clickable with agency
							...
							`<div class="btn-group">
								<input type="text" class="form-control text-center house-share-live-input" value="60" style="font-size: 12px !important;" readonly/>
							 </div>`,
