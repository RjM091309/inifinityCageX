...
function initNewGameGuestSelect2($guestSelect) {
	if (!$guestSelect || !$guestSelect.length) return;
	if ($guestSelect.data('select2')) {
		$guestSelect.select2('destroy');
	}
	$guestSelect.select2({
		placeholder: ($guestSelect.data('placeholder') || 'Select guest (optional)'),
		dropdownParent: '#modal-new-game-list',
		width: '100%'
	});
}

function resetNewGameGuestSelect() {
	var $guestSelect = $('#txtGuestGame');
	if (!$guestSelect.length) return;
	if ($guestSelect.data('select2')) {
		$guestSelect.select2('destroy');
	}
	$guestSelect.removeAttr('data-readonly data-locked-value');
...
function loadGuestsForSelectedAccount(preselectGuestId) {
	...
	var guestToPreselect = preselectGuestId || window._pendingNewGameGuestId || null;
	...
	$.ajax({
		...
		success: function (rows) {
			...
			guests.forEach(function (guest) {
				var label = (guest.guest_name || '').trim();
				if (guest.membership_no) {
					label = label ? (label + ' (' + guest.membership_no + ')') : String(guest.membership_no);
				}
			...
			if (guestToPreselect && $guestSelect.find('option[value="' + String(guestToPreselect) + '"]').length) {
				$guestSelect.val(String(guestToPreselect)).trigger('change.select2').trigger('change');
				$guestSelect.attr('data-readonly', '1');
				$guestSelect.attr('data-locked-value', String(guestToPreselect));
				window._pendingNewGameGuestId = null;
			} else {
...
function appendNewGameAccountOption($select, option) {
	var $opt = $('<option>', {
...
function addGameList(id) {
	...
	function populateOptions() {
		...
		if (Array.isArray(_accountOptionsCache) && _accountOptionsCache.length > 0) {
			_accountOptionsCache.forEach(function (option) {
				appendNewGameAccountOption($select, option);
			});
		}
	...
	if (typeof window.refreshNewGameAvailableChips === 'function') {
		window.refreshNewGameAvailableChips();
	}
	resetNewGameGuestSelect();
...
function computeGameListRowTotalsFromRecords(records, commissionType, commissionPercentage) {
	var total_buy_in = 0;
	...
	(records || []).forEach(function (res) {
		...
		if (res.CAGE_TYPE == 5) {
			var rollerTransaction = res.ROLLER_TRANSACTION || 1;
			if (rollerTransaction == 1) {
	...
	if (commissionType == 1 || commissionType == 3) {
		net = Math.round((total_rolling_chips * commissionPercentage) / 100);
	} else if (commissionType == 2) {
		net = Math.round((WinLoss * commissionPercentage) / 100);
	}

	return {
		hasMarkerBuyIn: hasMarkerBuyIn,
		total_initial: total_initial,
		total_buy_in_chips: total_buy_in_chips,
		total_cash_out_chips: total_cash_out_chips,
		total_rolling_chips: total_rolling_chips,
		total_rolling_real_chips: total_rolling_real_chips,
		total_roller_chips: total_roller_chips,
...
function gameListMarkerBuyinInlineStyle(hasMarker) {
	return hasMarker ? 'background-color:#FDEAEA !important;color:#C44545 !important;' : '';
}

function resolveGameListRowTotals(row, recordTotals) {
	var src = recordTotals || {};
	var total_rolling_chips = Number(src.total_rolling_chips) || 0;
	var WinLoss = Number(src.WinLoss) || 0;
	var net = 0;
	...
	return {
		hasMarkerBuyIn: !!src.hasMarkerBuyIn,
		total_initial: Number(src.total_initial) || 0,
		total_buy_in_chips: Number(src.total_buy_in_chips) || 0,
		total_cash_out_chips: Number(src.total_cash_out_chips) || 0,
		total_rolling_chips: total_rolling_chips,
		total_rolling_real_chips: Number(src.total_rolling_real_chips) || 0,
		total_roller_chips: Number(src.total_roller_chips) || 0,
		total_amount: Number(src.total_amount) || 0,
		WinLoss: WinLoss,
		net: net,
		winloss: WinLoss.toLocaleString()
	};
}
...
function bumpAssignGameGuestChildModalStack($childModal) {
	...
	if (!$childModal || !$childModal.length) return;
...
/** Keep a single backdrop when nested assign-guest child modals close. */
function syncAssignGuestModalBackdropStack() {
	if (typeof window.syncModalBackdropStack === 'function') {
		window.syncModalBackdropStack();
	}
}
...
function appendAssignGameGuestOption($guestSelect, guest) {
	...
	$opt.attr('data-guest-telegram', guest.guest_telegram || '');
...
function openEditGuestFromAssignGameGuest() {
	...
	$('#edit_guest_telegram_input').val($option.attr('data-guest-telegram') || '');
...
function gameListStripHtmlCell(val) {
	if (!val) return '';
...
$(document).ready(function () {
    ...
    /** Settlement mode: itago prev/next/badge kapag pick ng range (iba ang start at end). */
	...
	function buildDailySettleCheckbox(gameListId) {
		return `<label class="daily-settle-checkbox-wrap" title="Select game ${gameListId} to assign settlement date">
			<input type="checkbox" class="daily-settle-checkbox" value="${gameListId}" />
		</label>`;
	}
	...
	$.fn.dataTable.ext.type.order['game-list-col2-pre'] = function (d) {
		if (!d) return 0;
		var text = gameListStripHtmlCell(d);
		var m = text.match(/(\d+)/);
		return m ? parseInt(m[1], 10) : 0;
	};

	$.fn.dataTable.ext.type.order['game-list-num-pre'] = function (d) {
		var n = gameListParseNumericCell(gameListStripHtmlCell(d));
		return n !== null ? n : 0;
	};

	$.fn.dataTable.ext.type.order['game-list-text-pre'] = function (d) {
		return gameListStripHtmlCell(d).toLowerCase();
	};

	$.fn.dataTable.ext.type.order['game-list-date-pre'] = function (d) {
		var text = gameListStripHtmlCell(d);
		if (!text) return 0;
		var m = moment(text, 'MMM DD, HH:mm', true);
		if (m.isValid()) return m.valueOf();
		m = moment(text, 'MMM DD, YYYY HH:mm', true);
		if (m.isValid()) return m.valueOf();
		return 0;
	};
	...
	function accumulateGameListRowIntoTotals(totals, data) {
		var tempDiv = $('<div>');
		var amountText = tempDiv.html(data[5] || '0').text().trim();
		tempDiv = $('<div>');
		var chipsReturnText = tempDiv.html(data[6] || '0').text().trim();
		tempDiv = $('<div>');
		var totalRollingText = tempDiv.html(data[7] || '0').text().trim();
		tempDiv = $('<div>');
		...
		totals.totalRolling += parseFloat(totalRollingText.replace(/[^0-9.-]/g, '')) || 0;
	...
	var dataTable = $('#game_list-tbl').DataTable({
		...
		columnDefs: [
			{ targets: 0, type: 'game-list-date', className: 'text-center col-game-start' },
			{ targets: 1, type: 'game-list-text', className: 'text-center col-type' },
			{ targets: 2, type: 'game-list-col2', className: 'text-center col-game-no' },
			{ targets: 3, type: 'game-list-text', className: 'text-center col-acct-no' },
			{ targets: 4, type: 'game-list-text', className: 'text-center col-guest' },
			{ targets: 5, type: 'game-list-num', className: 'text-center col-buyin' },
			{ targets: 6, type: 'game-list-num', className: 'text-center col-cash-out' },
			{ targets: 7, type: 'game-list-num', className: 'text-center col-total-rolling' },
			{ targets: 8, type: 'game-list-num', className: 'text-center col-roller-chips' },
			{ targets: 9, type: 'game-list-num', className: 'text-center' },
			{ targets: 10, type: 'game-list-num', className: 'text-center col-commission' },
			{ targets: 11, type: 'game-list-num', className: 'text-center col-winloss' },
			{ targets: 12, type: 'game-list-text', className: 'text-center' },
			{ targets: 13, type: 'game-list-text', className: 'text-center col-game-end' },
		...
		createdRow: function (row, data, index) {
			...
			if (parseInt(data[11].split(',').join('')) < 0) {
				$('td:eq(11)', row).css({
	...
	var GAME_LIST_EXPORT_COL_WIDTHS = [16, 11, 10, 28, 20, 14, 14, 16, 14, 12, 14, 14, 12, 18];
	/* 1-based Excel columns: money / numeric amounts (not GAME # col 3) */
	var GAME_LIST_EXPORT_RIGHT_AMOUNT_COLS = [6, 7, 8, 10, 11];
	...
	function coerceGameListExportCell(colIdx0, plainText) {
		...
		var numericCols0 = [5, 6, 7, 8, 10, 11];
	...
	window.showAccountSummaryView = function () {
		...
		var dt = $('#game_list-tbl').DataTable();
		dt.clear();
		var grandAmount = 0, grandChipsReturn = 0, grandRolling = 0, grandRollerChips = 0, grandCommission = 0, grandWinLoss = 0;
    ...
    var gameListReloadGeneration = 0;

    function isGameListReloadStale(gen) {
        return gen !== gameListReloadGeneration;
    }
    ...
    function clearGameListDisplay() {
        ...
        dataTable.clear();
        dataTable.draw();
        $('#game_list-tbl tfoot #GRAND_TOTAL_AMOUNT, #GRAND_CHIPS_RETURN, #GRAND_TOTAL_ROLLING, #GRAND_ROLLER_CHIPS, #GRAND_COMMISSION, #GRAND_WIN_LOSS').text('0.00');
    }

    function reloadData() {
        gameListReloadGeneration++;
        var currentReloadGen = gameListReloadGeneration;
        ...
        $.ajax({
            ...
            success: function (data) {
                if (isGameListReloadStale(currentReloadGen)) return;
                ...
                function onGameListRecordsBatchComplete() {
                    if (isGameListReloadStale(currentReloadGen)) return;
                    if (!hasAccountSearch) {
                        dataTable.draw();
                    }
...
// let rowClass = isHighlighted ? 'highlight-row' : '';
                    ...
                    var btn_his = `<div class="btn-group" role="group">
                        <button type="button" onclick="showHistory(${row.game_list_id}, '${(row.agent_code || '').replace(/'/g, "\\'")}')" class="btn btn-sm btn-info-subtle action-btn-square js-bs-tooltip-enabled"
                    ...
                    var btn_services = `<div class="btn-group" role="group">
                        <button type="button" onclick="openServices(${row.game_list_id}, '${encodeURIComponent(row.agent_name || '')}', ${row.game_status}, ${row.SETTLED || 0}, ${row.AGENT_ID || 0}, '${(row.agent_code || '').replace(/'/g, "\\'")}')" class="btn btn-sm btn-primary-subtle action-btn-square js-bs-tooltip-enabled"
                    ...
                    var resolvedTotals = resolveGameListRowTotals(row, row.record_totals);
                    var hasMarkerBuyIn = resolvedTotals.hasMarkerBuyIn;
                    var total_initial = resolvedTotals.total_initial;
                    var total_buy_in_chips = resolvedTotals.total_buy_in_chips;
                    var total_cash_out_chips = resolvedTotals.total_cash_out_chips;
                    var total_rolling_chips = resolvedTotals.total_rolling_chips;
                    var total_rolling_real_chips = resolvedTotals.total_rolling_real_chips;
                    var total_roller_chips = resolvedTotals.total_roller_chips;
                    var total_amount = resolvedTotals.total_amount;
                    var winloss = resolvedTotals.winloss;
                    var WinLoss = resolvedTotals.WinLoss;
                    var net = resolvedTotals.net;
							...
							var cashout_td = '';
							var roller_chips_td = '';
							var totalRollingDisplay = parseFloat(total_rolling_chips).toLocaleString();
							var rollingOnclick = 'addRolling(' + row.game_list_id + ', \'' + (row.agent_code || '').replace(/'/g, "\\'") + '\')';
							var isInitialBuyinMarker = hasMarkerBuyIn;
							var buyinAmountDisplay = parseFloat(total_amount).toLocaleString();
							var buyinInlineStyle = gameListMarkerBuyinInlineStyle(isInitialBuyinMarker);
							...
							if (row.game_status == 2) {
								...
								buyin_td = '<button class="btn btn-link" style="font-size:11px;text-decoration: underline;' + buyinInlineStyle + '" onclick="addBuyin(' + row.game_list_id + ', ' + row.ACCOUNT_ID + ', \'' + (row.agent_code || '').replace(/'/g, "\\'") + '\')">' + buyinAmountDisplay + '</button>';
								total_rolling_td = '<button class="btn btn-link" style="font-size:11px;text-decoration: underline;" onclick="' + rollingOnclick + '">' + totalRollingDisplay + '</button>';
								...
								if (isSettled) {
									...
									total_rolling_td = totalRollingDisplay;
								...
								<button type="button" onclick="settlement_history(${row.game_list_id}, ${row.ACCOUNT_ID }, '${(row.agent_code || '').replace(/'/g, "\\'")}')" class="btn btn-sm ${settleClass} action-btn-square js-bs-tooltip-enabled"
								...
								if (userPermissions === 11 || userPermissions === 1 || userPermissions === 0) { // If manager or Super admin
									...
									if (isSettled && userPermissions !== 0) { // Super admin (0) can edit even when settled
										status = `<a href="#" class="${statusDateClass}" style="font-size:10px !important;" aria-label="Status" data-bs-toggle="tooltip" data-bs-original-title="${settledTooltip}" onclick="showSettledAlert(); return false;">${moment(row.GAME_ENDED).format('MMM DD, HH:mm')}</a>`;
									} else {
										status = `<a href="#" class="${statusDateClass}" style="font-size:10px !important;" onclick="${gameListChangeStatusOnclick(row, net, row.ACCOUNT_ID, total_amount, total_cash_out_chips, total_rolling_chips, WinLoss, null)}">${moment(row.GAME_ENDED).format('MMM DD, HH:mm')}</a>`;
									}

								} else {
									
								// //END GAME STATUS NOT EDITABLE
								status = `<a href="#" onclick="showEndGameAlert()">${moment(row.GAME_ENDED).format('MMM DD, HH:mm')}</a>`;
						   ...
						   var acct_no_link = `<a href="#" onclick="account_details(${row.ACCOUNT_ID}, '${row.agent_code}', '${row.agent_name}')">${row.agent_code} (${row.agent_name})</a>`;
						   tableRowsBatch.push([gameStartCell, buildGameTypeCell(row), buildCutoffGameIdCell(row), acct_no_link, buildGameGuestCell(row), buyin_td, cashout_td, total_rolling_td, roller_chips_td, buildGameRateCell(row, userPermissions, isSettled), formattedNet, winloss, translateGameSource(row.INITIAL_MOP), status, actionButtons]);
                ...
                if (tableRowsBatch.length) {
                    dataTable.rows.add(tableRowsBatch);
                }

                onGameListRecordsBatchComplete();
            },
    ...
    // Settle button state (Undo removed by request)
    window.updateSettleButtonState = function (recordCount) {
        ...
        // Allow Settle action on all settlement-date views.
    ...
    function navigateToDate(targetDate, preferredSubView) {
        ...
        // Update flatpickr settlement range (single day = collapsed range)
    ...
    $('#btn-settlement-prev').on('click', function() {
        ...
        // Dual-state navigation on current date:
        // Previous from Open -> Settled on the same date.
        if (
            !window.selectedSettlementRangeMultiDay &&
    ...
    $('#btn-settlement-next').on('click', function() {
        ...
        // Dual-state navigation on current date:
        // Next from Settled -> Open on the same date.
        if (
            !window.selectedSettlementRangeMultiDay &&
        ...
        if (nextDate) {
            // When moving into current date from older dates, show settled first
            // if settled rows exist; next press will switch to Open on same date.
    ...
    if (!didRestoreDailySettleSession) {
        // Keep OPEN pinned to local current date unless an explicit session state was restored.
	...
	$('#add_cashout').submit(function (event) {
		...
		if (splitEnabled) {
			...
			var commonPayload = {
				...
				dont_send_telegram: $('#dontSendCashoutTelegram').is(':checked') ? '1' : '0'
			};
...
function addBuyin(id, account, agentCode) {
	...
	$('#add-buyin-agent-code').text(agentCode || '');
...
function addRolling(id, agentCode) {
	$('#add-rolling-agent-code').text(agentCode || '');
...
function addRollerChips(id, returnOnly, agentCode) {
	$('#add-roller-chips-agent-code').text(agentCode || '');
	$('#modal-add-roller-chips').modal('show');
...
function addCashout(id, account, total_rolling_chips, agentCode) {
	$('#add-cashout-agent-code').text(agentCode || '');
	...
	$('#dontSendCashoutTelegram').prop('checked', false);
	if (splitRow) {
...
function showHistory(id, agentCode) {
	record_id = id;
	$('#show-rolling-agent-code').text(agentCode || '');
...
function reloadDataRecord() {
    $.ajax({
        ...
        method: 'GET',
        success: function (data) {
            if (data.length > 0 && data[0].agent_code) {
                $('#show-rolling-agent-code').text(data[0].agent_code);
            }
...
function openServices(id, guestName, gameStatus, settled, agentId, agentCode) {
	// Track settled state
	...
	$('#services-agent-code').text(agentCode || '');
...
// Delete game (Super Admin only) - soft delete game_list, game_record, account_ledger; excludes game_services & daily_settlement
					...
					if (rollerTransaction == 1) {
						...
						total_roller_return_cc += (res.ROLLER_CC_CHIPS || 0);
							...
							if (row.game_status == 2) {
								...
								rolling_td = '<button class="btn btn-link" style="font-size:11px;text-decoration: underline;" onclick="addRolling(' + row.game_list_id + ', \'' + (row.agent_code || '').replace(/'/g, "\\'") + '\')">' + parseFloat(total_rolling_real_chips).toLocaleString() + '</button>';
								cashout_td = '<button class="btn btn-link" style="font-size:11px;text-decoration: underline;" onclick="addCashout(' + row.game_list_id + ', ' + row.ACCOUNT_ID + ', ' + total_rolling_chips + ', \'' + (row.agent_code || '').replace(/'/g, "\\'") + '\')">' + parseFloat(total_cash_out_chips).toLocaleString() + '</button>';
...
function settlement_history(record_id, acc_id, agentCode) {
    $('#settlement-agent-code').text(agentCode || '');
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (data.length > 0) {
                    ...
                    $('#settlement-agent-code').text(data[0].agent_code || '');
...
$('#txtTrans').on('change', function () {
    ...
    loadGuestsForSelectedAccount(window._pendingNewGameGuestId || null);
...
$(document).ready(function () {
	...
	$('#add_guest_form').on('submit', function (e) {
		...
		$.ajax({
			...
			success: function (res) {
				...
				loadAssignGameGuestSelect(agentId, newGuestId);
				Swal.fire({
					icon: 'success',
					title: 'Success',
	...
	$('#modal-assign-game-guest').on('hidden.bs.modal', function () {
		...
		requestAnimationFrame(function () {
			syncAssignGuestModalBackdropStack();
		});
	});
	...
	$(document).on('click', '#btn-assign-guest-game-history', function () {
		...
		var historyPromise = window.game_history(accountId, guestId);
		if (historyPromise && typeof historyPromise.catch === 'function') {
			historyPromise.catch(function (err) {
		...
		$.ajax({
			...
			error: function (xhr) {
				Swal.fire({
					icon: 'error',
					title: 'Error',
					text: (xhr.responseJSON && xhr.responseJSON.error) || 'Failed to save guest.'
				});
			},
			complete: function () {
				updateAssignGameGuestSaveState();
