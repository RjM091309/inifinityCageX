...
// Pending passport extraction (used to auto-fill New Guest before save)
...
function setGuestSubmitButtonsDisabled(disabled) {
	$('#submit-new-agent-btn').prop('disabled', !!disabled);
}
...
function buildLandmarkDrivenRect(positions, canvasW, canvasH, offsetY) {
	...
	var top = browTop - faceWidth * 1.0;
	var bottom = yf(chin.y) + faceWidth * 0.45;
	var srcH = bottom - top;
...
async function extractPassportFromFile(file) {
	...
	async function toReasonableJpegDataUrl(inputDataUrl) {
		try {
			var img = await new Promise(function (resolve, reject) {
				...
				el.onerror = function () { reject(new Error('Failed to load image')); };
				el.src = inputDataUrl;
			});
			var maxDim = 1600;
			var w = img.naturalWidth || img.width;
	...
	// Always downscale/compress before API — keeps payload small and avoids 413 errors
	...
	$('#add_new_agent').on('submit', function(event) {
		...
		$.ajax({
			...
			success: function(response) {
				Swal.fire({
					title: 'Success!',
					text: (response && response.message) ? response.message : 'Guest saved successfully.',
					icon: 'success',
			...
			error: function(xhr) {
				var errText = (xhr.responseJSON && xhr.responseJSON.error)
