...
$(document).ready(function() {
    ...
    function initializeExpenseTable() {
        ...
        function reloadData() {
            ...
            // Swap if user clicked the later date first
            if (start > end) {
                [start, end] = [end, start];
            }
            ...
            $.ajax({
                ...
                success: function(data) {
                        ...
                        if (permissions !== 2) {
                            btn = `
                                <div class="btn-group">
                                    ...
                                    <button type="button" class="btn btn-sm btn-alt-secondary"
                                            onclick="edit_expense(${row.expense_id}, '${row.expense_category_id}', '${row.RECEIPT_NO}', '${row.DATE_TIME || row.ENCODED_DT}', '${row.DESCRIPTION}', '${row.AMOUNT}', '${row.OIC}')"
                    ...
                    $('#GRAND_TOTAL_AMOUNT').text(`₱${total_expense.toLocaleString()}`);
                },
