 ...
 *
 * Extracted verbatim (same formulas, same call order) from routes/net_profit.js so both
 * consumers always agree — a fix or formula change here automatically applies to both without
 * needing a second edit.
 *
 * NOTE: this app computes net profit **per settlement date** (daily_settlement /
 * daily_settlement_games), not per game_list.PROGRAM_DATE directly — keep that distinction in
 * mind if comparing against another cage app's implementation of the same idea.
 */
...
/** Games currently pending daily settlement (not yet posted to daily_settlement_games) — used to fold "live today" into the current settlement date. */
...
/**
 * Per-settlement-date net profit rows for [startStr, endStr] — same computation as the
 * Net Profit page's Daily view (routes/net_profit.js GET /net_profit_data).
 * One row per distinct settlement date: { settlement_date, settlement_label, game_count,
 * win_loss, share_percentage, casino_share, commission, house_expenses_settled,
 * grand_net_profit }.
 */
async function computeNetProfitRows(startStr, endStr) {
	...
	let distinctDates = await loadDistinctSettlementDatesInRange(startStr, endStr);
...
/** Sum of computeNetProfitRows(startStr, endStr) into one totals object (mirrors the Net Profit page's range_totals, minus share_percentage which is display-only there). */
...
module.exports = {
	...
	loadDistinctSettlementDatesInRange,
